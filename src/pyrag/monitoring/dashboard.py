...
class MonitoringDashboard:
    ...
    async def start(self):
        ...
        if not self.current_layout and self.layouts:
            default_layout = next(
                (l for l in self.layouts if l.default), self.layouts[0]
            )
    ...
    async def _get_metrics_table_data(self, widget: DashboardWidget) -> Dict[str, Any]:
        ...
        for metric_key, label, unit, format_type in metrics_to_show:
            ...
            data.append(
                {"metric": label, "value": value, "unit": unit, "status": status}
            )

        return {"type": "table", "data": data, "columns": columns}
    ...
    async def _start_web_server(self):
        ...
        logger.info(
            f"Dashboard web server would start on {self.config.host}:{self.config.port}"
        )
