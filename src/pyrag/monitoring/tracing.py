...
class TraceCollector:
    ...
    def create_trace(self, correlation_id: Optional[str] = None) -> Trace:
        ...
        trace = Trace(
            trace_id=trace_id, correlation_id=correlation_id, start_time=time.time()
        )
    ...
    def get_recent_traces(self, hours: int = 1) -> List[Trace]:
        ...
        return [
            trace for trace in self.traces.values() if trace.start_time > cutoff_time
        ]
    ...
    def decorator(func):
        async def wrapper(*args, **func_kwargs):
            ...
            try:
                ...
                collector.end_span(
                    span.span_id, {"error": True, "error.message": str(e)}
                )
                raise
