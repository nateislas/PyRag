...
class MonitoringSystem:
    ...
    async def _cleanup_old_metrics(self):
        ...
        logger.debug(
            f"Cleaned up metrics, retained {len(self.metrics_history)} entries"
        )
    ...
    async def trace_operation(self, operation_name: str, **kwargs):
        ...
        span = self.trace_collector.create_span(
            name=operation_name, trace_id=trace_id, tags=kwargs
        )
        try:
            yield span
        finally:
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        if self.alert_manager:
            status["components"]["alert_manager"] = (
                "healthy" if self.alert_manager.is_running() else "stopped"
            )

        if self.health_monitor:
            status["components"]["health_monitor"] = (
                "healthy" if self.health_monitor.is_running() else "stopped"
            )

        if self.trace_collector:
            status["components"]["trace_collector"] = (
                "healthy" if self.trace_collector.is_running() else "stopped"
            )

        if self.structured_logger:
            status["components"]["structured_logger"] = (
                "healthy" if self.structured_logger.is_running() else "stopped"
            )

        if self.prometheus_exporter:
            status["components"]["prometheus_exporter"] = (
                "healthy" if self.prometheus_exporter.is_running() else "stopped"
            )
