...
class PrometheusExporter:
    ...
    def add_metric(
        ...
        self.metrics[name] = PrometheusMetric(
            name=name, metric_type=metric_type, labels=labels, buckets=buckets or []
        )
    ...
    async def _start_web_server(self):
        ...
        logger.info(
            f"Prometheus metrics endpoint would be available at http://{self.config.host}:{self.config.port}{self.config.path}"
        )
...
class PrometheusMetric:
    ...
    def __init__(
        self, name: str, metric_type: str, labels: List[str], buckets: List[float]
    ):
        self.name = name
    ...
    def record_value(self, value: float, labels: Dict[str, str]):
        ...
        # Create metric value
        metric_value = MetricValue(value=value, labels=labels, timestamp=time.time())
