...
class SecurityConfig:
    ...
    def __init__(self):
        self.enable_rate_limiting = os.getenv("MCP_ENABLE_RATE_LIMIT", "true").lower() == "true"
        self.rate_limit_requests = int(os.getenv("MCP_RATE_LIMIT_REQUESTS", "100"))  # requests per window
        self.rate_limit_window = int(os.getenv("MCP_RATE_LIMIT_WINDOW", "3600"))  # seconds (1 hour)
        self.enable_api_keys = os.getenv("MCP_ENABLE_API_KEYS", "false").lower() == "true"
        self.api_key_secret = os.getenv("MCP_API_KEY_SECRET", "your-secret-key-change-this")
        self.max_request_size = int(os.getenv("MCP_MAX_REQUEST_SIZE", "1048576"))  # 1MB
        self.allowed_ips = os.getenv("MCP_ALLOWED_IPS", "").split(",") if os.getenv("MCP_ALLOWED_IPS") else []
        self.enable_ip_whitelist = os.getenv("MCP_ENABLE_IP_WHITELIST", "false").lower() == "true"
...
def validate_api_key(api_key: str, timestamp: str, signature: str) -> bool:
    ...
    try:
        ...
        if abs((datetime.now(request_time.tzinfo) - request_time).total_seconds()) > 300:
            return False
...
def check_rate_limit(client_id: str) -> bool:
    ...
    while client_requests and client_requests[0] < now - security_config.rate_limit_window:
    ...
    try:
        ...
        if security_config.enable_api_keys:
            ...
            if not all([api_key, timestamp, signature]):
                logger.warning(f"Missing authentication parameters from IP: {client_ip}")
                return False
