...
class FirecrawlClient:
    ...
    async def scrape_url(self, url: str, options: Optional[Dict[str, Any]] = None) -> ScrapedDocument:
    ...
    async def crawl_site(self, start_url: str, options: Optional[Dict[str, Any]] = None) -> List[ScrapedDocument]:
    ...
    def _parse_extract_response(self, data: Dict[str, Any], url: str) -> ScrapedDocument:
    ...
    def _parse_extract_site_response(self, data: Dict[str, Any]) -> List[ScrapedDocument]:
    ...
    async def _poll_job_status(self, job_id: str, url: str, max_attempts: int = 60, delay: float = 3.0) -> ScrapedDocument:
        """Poll job status until completion."""
        for attempt in range(max_attempts):
            try:
                async with self.session.get(
                    f"{self.base_url}/v2/extract/{job_id}",
                    headers=self.headers
                ) as response:
                    if response.status == 200:
                        data = await response.json()
                        ...
                        if data.get("status") == "completed":
                            ...
                            # Still processing
                            logger.info(f"Job {job_id} still processing (attempt {attempt + 1}/{max_attempts})")
                            await asyncio.sleep(delay)
                    else:
                        error_text = await response.text()
                        logger.error(f"Error checking job status: {response.status} - {error_text}")
