...
from .sitemap_analyzer import SitemapAnalyzer, SitemapAnalysis
...
class IntelligentCrawler:
    ...
    async def crawl_documentation_site(
        ...
        if not sitemap_analysis:
            ...
            sitemap_analysis = await sitemap_analyzer.analyze_documentation_site(base_url)
        
        if not structure:
            urls = [entry.url for entry in sitemap_analysis.discovered_urls]
            structure = self.structure_mapper.map_documentation_structure(urls, base_url)
    ...
    def _initialize_crawl_state(self, sitemap_analysis: SitemapAnalysis, structure: DocumentationStructure):
    ...
    def _get_crawl_priorities(
        ...
        if self.strategy.name == "selective":
            ...
            priorities = [
                url for url in priorities 
                if self.importance_scores.get(url, 0) > self.strategy.importance_threshold
        ...
        for content_type, urls in structure.content_types.items():
            if content_type not in ["other", "detail"]:  # Skip generic types
                ...
                type_priorities = sorted(
                    urls, 
                    key=lambda u: self.importance_scores.get(u, 0), 
                    reverse=True
                )[:10]
    ...
    async def _execute_crawl_strategy(self, priority_urls: List[str], structure: DocumentationStructure):
    ...
    async def _crawl_single_url(self, url: str, structure: DocumentationStructure):
        ...
        try:
            self.logger.info(f"🔍 Analyzing URL {self.progress.total_crawled + 1}/{len(self.discovered_urls)}: {url}")
        ...
        for link in links:
            try:
                ...
                if link_domain == base_domain or link_domain.endswith(f".{base_domain}"):
                    ...
                    if self._is_documentation_url(link) and link not in self.discovered_urls:
                        new_urls.add(link)
    ...
    async def _should_continue_at_depth(self, depth: int, urls: List[str]) -> bool:
        ...
        should_continue = (
            avg_quality >= self.strategy.content_quality_threshold and
            avg_importance >= self.strategy.importance_threshold
        )
    ...
    async def _crawl_depth_level(self, urls: List[str], structure: DocumentationStructure):
    ...
    def _compile_crawl_result(self) -> CrawlResult:
        ...
        return CrawlResult(
            ...
            success=len(self.progress.errors) < 10,  # Consider successful if < 10 errors
        )
    ...
    def get_crawl_recommendations(self) -> Dict[str, Any]:
        ...
        if self.content_quality_scores:
            avg_quality = sum(self.content_quality_scores.values()) / len(self.content_quality_scores)
            if avg_quality < 0.6:
