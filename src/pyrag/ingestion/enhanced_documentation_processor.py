...
class EnhancedDocumentationProcessor:
    ...
    async def process_scraped_document(
        ...
        try:
            self.logger.info(f"Processing document with enhanced system: {scraped_doc.url}")
            ...
            stats = {
                ...
                "semantic_boundaries": sum(len(chunk.semantic_chunk.semantic_boundaries) for chunk in enhanced_chunks),
                "rich_metadata_extracted": any(chunk.enhanced_metadata.parameters or chunk.enhanced_metadata.return_type for chunk in enhanced_chunks)
            }
            ...
            self.logger.error(f"Error in enhanced processing of document {scraped_doc.url}: {e}")
    ...
    def _extract_enhanced_metadata(self, enhanced_chunks: List['EnhancedDocumentChunk']) -> Dict[str, Any]:
        ...
        for chunk in enhanced_chunks:
            ...
            if chunk.semantic_chunk.function_signature:
                metadata["function_signatures"].append(chunk.semantic_chunk.function_signature)
    ...
    def get_processing_stats(self, results: List[EnhancedProcessingResult]) -> Dict[str, Any]:
        ...
        total_enhanced_chunks = sum(r.processing_stats.get("enhanced_chunks", 0) for r in results)
