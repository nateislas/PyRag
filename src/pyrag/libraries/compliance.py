...
class ComplianceTracker:
    ...
    async def check_library_compliance(self, library_info: Dict[str, Any]) -> ComplianceStatus:
        ...
        try:
            ...
            if existing_record:
                ...
                existing_record.compliance_score = self._calculate_compliance_score(existing_record)
            ...
            self.logger.error(f"Failed to check compliance for {library_info.get('name', 'unknown')}: {e}")
    ...
    async def contact_maintainer(self, library_name: str, contact_info: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            compliance_record.compliance_score = self._calculate_compliance_score(compliance_record)
    ...
    async def get_compliance_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            compliant_libraries = sum(1 for record in self.compliance_records.values() if record.compliance_score >= 0.7)
            non_compliant_libraries = sum(1 for record in self.compliance_records.values() if record.compliance_score < 0.7)
            pending_opt_outs = sum(1 for record in self.compliance_records.values() if record.opt_out_status == "requested")
            
            avg_compliance_score = sum(record.compliance_score for record in self.compliance_records.values()) / total_libraries
    ...
    async def get_library_compliance(self, library_name: str) -> Optional[ComplianceStatus]:
    ...
    async def update_license_info(self, library_name: str, new_license: str) -> bool:
        ...
        try:
            ...
            compliance_record.license_approved = normalized_license in self.approved_licenses
            compliance_record.last_updated = datetime.now()
            compliance_record.compliance_score = self._calculate_compliance_score(compliance_record)
