...
class LibraryManager:
    ...
    async def add_library_automated(self, library_name: str) -> bool:
        ...
        try:
            ...
            if not availability["available"]:
                self.logger.warning(f"Library {library_name} is not available: {availability['reason']}")
                return False
    ...
    async def remove_library(self, library_name: str) -> bool:
        ...
        try:
            if library_name in self.managed_libraries:
                ...
                self.logger.warning(f"Library {library_name} not found in managed libraries")
                return False
    ...
    async def update_library_info(self, library_name: str) -> bool:
        ...
        try:
            ...
            if library_info:
                self.managed_libraries[library_name] = library_info
                self.logger.info(f"Successfully updated library info for {library_name}")
                return True
            else:
                self.logger.error(f"Failed to get updated info for library {library_name}")
                return False
    ...
    async def get_library_stats(self) -> Dict[str, Any]:
        ...
        if total_libraries > 0:
            avg_quality = sum(lib.quality_score for lib in self.managed_libraries.values()) / total_libraries
            avg_popularity = sum(lib.popularity_score for lib in self.managed_libraries.values()) / total_libraries
        else:
            avg_quality = 0.0
            avg_popularity = 0.0
        
        # Count libraries by quality tier
        high_quality = sum(1 for lib in self.managed_libraries.values() if lib.quality_score > 0.7)
        medium_quality = sum(1 for lib in self.managed_libraries.values() if 0.4 <= lib.quality_score <= 0.7)
        low_quality = sum(1 for lib in self.managed_libraries.values() if lib.quality_score < 0.4)
