...
class LibraryDiscovery:
    ...
    async def discover_popular_libraries(self, limit: int = 50) -> List[LibraryInfo]:
        ...
        try:
            ...
            combined_libraries = self._combine_library_data(
                pypi_libraries, github_libraries
            )
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(f"{self.pypi_stats_url}/packages") as response:
                    if response.status == 200:
                        ...
                        self.logger.warning(
                            f"Failed to get PyPI stats: {response.status}"
                        )
                        return []
    ...
    def _combine_library_data(
        self, pypi_libraries: List[Dict], github_libraries: List[Dict]
    ) -> List[Dict[str, Any]]:
    ...
    async def _rank_libraries_by_quality(
        ...
        ranked_libraries.sort(
            key=lambda x: x.quality_score + x.popularity_score, reverse=True
        )
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(
                    f"{self.pypi_base_url}/{library_name}/json"
                ) as response:
                    if response.status == 200:
                        data = await response.json()
                        ...
                        self.logger.warning(
                            f"Failed to get PyPI info for {library_name}: {response.status}"
                        )
                        return None
    ...
    async def check_library_availability(self, library_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not library_info:
                return {"available": False, "reason": "Library not found on PyPI"}
            ...
            self.logger.error(
                f"Error checking library availability for {library_name}: {e}"
            )
