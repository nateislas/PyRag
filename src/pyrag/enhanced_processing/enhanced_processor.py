...
from .metadata_extractor import EnhancedMetadata, MetadataExtractor
...
class EnhancedDocumentProcessor:
    ...
    async def process_document(
        self, content: str, metadata: Dict[str, Any], hierarchy_path: List[str]
        ...
        for i, semantic_chunk in enumerate(semantic_chunks):
            self.logger.info(
                f"Processing semantic chunk {i+1}/{len(semantic_chunks)}: {semantic_chunk.chunk_type}"
            )
            ...
            processing_stats["chunk_types"][semantic_chunk.chunk_type] = (
                processing_stats["chunk_types"].get(semantic_chunk.chunk_type, 0) + 1
            )
    ...
    async def _process_content_by_type(
        self, semantic_chunk: SemanticChunk
    ) -> Dict[str, Any]:
        ...
        if semantic_chunk.chunk_type == "function":
            ...
            api_doc = await self.content_processor.process_api_documentation(
            ...
            if semantic_chunk.examples:
                ...
                for example in semantic_chunk.examples:
                    example_chunk = await self.content_processor.process_example(
                        example
                    )
            ...
            if self._is_tutorial_content(semantic_chunk.content):
                tutorial_chunk = await self.content_processor.process_tutorial(
                    semantic_chunk.content
                )
        ...
        if code_blocks:
            ...
            content_processing["code_chunk"] = (
                code_chunks[0] if code_chunks else None
    ...
    def _calculate_metadata_completeness(
        self, chunks: List[EnhancedDocumentChunk]
    ) -> float:
    ...
    def to_vector_store_format(
        self, enhanced_chunk: EnhancedDocumentChunk
    ) -> Dict[str, Any]:
        ...
        return {"content": enhanced_chunk.content, "metadata": vector_metadata}
