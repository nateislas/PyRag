...
class RelationshipManager:
    ...
    async def expand_search_context(
        ...
        self.logger.info(f"Expanding context for {len(primary_results)} primary results")
        ...
        grouped_results = self._group_expanded_results(primary_results, expanded_results)
        ...
        self.logger.info(f"Context expansion added {len(expanded_results)} related documents")
    ...
    def _build_relationship_graph(self, results: List[Dict[str, Any]]) -> Dict[str, List[DocumentRelationship]]:
        ...
        for result in results:
            ...
            if "parent" in relationship_metadata:
                ...
                relationship_graph[doc_id].append(DocumentRelationship(
                    source_id=doc_id,
                    target_id=parent_id,
    ...
    def _calculate_expansion_metadata(
        ...
        avg_strength = total_strength / total_relationships if total_relationships > 0 else 0
    ...
    def discover_semantic_relationships(
        ...
        self.logger.info(f"Discovering semantic relationships between {len(documents)} documents")
        ...
        for i, doc1 in enumerate(documents):
            for j, doc2 in enumerate(documents[i+1:], i+1):
                ...
                if similarity >= similarity_threshold:
                    ...
                    relationship_type = self._determine_relationship_type(doc1, doc2, similarity)
    ...
    def _calculate_content_similarity(self, doc1: Dict[str, Any], doc2: Dict[str, Any]) -> float:
    ...
    def _determine_relationship_type(
        ...
        if content_type1 == "api_reference" and content_type2 in ["examples", "tutorials"]:
            return "parent"  # API reference is parent to examples/tutorials
        elif content_type1 in ["examples", "tutorials"] and content_type2 == "api_reference":
            return "child"   # Examples/tutorials are children of API reference
        
        # Default to related
        return "related"
    
    def create_relationship_summary(self, relationships: List[DocumentRelationship]) -> Dict[str, Any]:
