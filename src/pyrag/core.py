...
class PyRAG:
    ...
    def __init__(self):
        ...
        self.search_engine = EnhancedSearchEngine(self.vector_store, self.embedding_service)
    ...
    async def get_library_status(self, library_name: str) -> Optional[Dict[str, Any]]:
        ...
        with get_session() as session:
            library = session.query(Library).filter(Library.name == library_name).first()
            if not library:
                return None
    ...
    async def add_documents(
        ...
        try:
            ...
            with get_session() as session:
                ...
                library_id = library.id  # Get ID while library is still in session context
            ...
            with get_session() as session:
                ...
                library_version = session.query(LibraryVersion).filter(
                ...
                library_obj = session.query(Library).filter(Library.id == library_id).first()
                if library_obj:
                    library_obj.indexing_status = "completed"
                    library_obj.last_checked = None  # Will be updated by monitoring system
