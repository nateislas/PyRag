...
from .test_suite import TestResult, QueryCategory, DifficultyLevel
...
class EvaluationMetrics:
    ...
    def _calculate_average_scores(self, test_results: List[TestResult]) -> None:
        ...
        self.average_relevance = sum(r.relevance_score or 0 for r in successful_results) / len(successful_results)
        self.average_accuracy = sum(r.accuracy_score or 0 for r in successful_results) / len(successful_results)
        self.average_completeness = sum(r.completeness_score or 0 for r in successful_results) / len(successful_results)
        self.average_helpfulness = sum(r.helpfulness_score or 0 for r in successful_results) / len(successful_results)
        self.average_overall = sum(r.overall_score or 0 for r in successful_results) / len(successful_results)
    ...
    def _calculate_breakdowns(self, test_results: List[TestResult]) -> None:
        ...
        for category in QueryCategory:
            category_results = [r for r in test_results if r.test_case.category == category and r.is_successful()]
            if category_results:
                self.category_scores[category.value] = {
                    "count": len(category_results),
                    "average_relevance": sum(r.relevance_score or 0 for r in category_results) / len(category_results),
                    "average_accuracy": sum(r.accuracy_score or 0 for r in category_results) / len(category_results),
                    "average_completeness": sum(r.completeness_score or 0 for r in category_results) / len(category_results),
                    "average_helpfulness": sum(r.helpfulness_score or 0 for r in category_results) / len(category_results),
                    "average_overall": sum(r.overall_score or 0 for r in category_results) / len(category_results),
                }
        
        # Difficulty breakdown
        for difficulty in DifficultyLevel:
            difficulty_results = [r for r in test_results if r.test_case.difficulty == difficulty and r.is_successful()]
            if difficulty_results:
                self.difficulty_scores[difficulty.value] = {
                    "count": len(difficulty_results),
                    "average_relevance": sum(r.relevance_score or 0 for r in difficulty_results) / len(difficulty_results),
                    "average_accuracy": sum(r.accuracy_score or 0 for r in difficulty_results) / len(difficulty_results),
                    "average_completeness": sum(r.completeness_score or 0 for r in difficulty_results) / len(difficulty_results),
                    "average_helpfulness": sum(r.helpfulness_score or 0 for r in difficulty_results) / len(difficulty_results),
                    "average_overall": sum(r.overall_score or 0 for r in difficulty_results) / len(difficulty_results),
                }
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "evaluation_info": {
                "started": self.evaluation_started.isoformat() if self.evaluation_started else None,
                "completed": self.evaluation_completed.isoformat() if self.evaluation_completed else None,
    ...
    def get_improvement_areas(self) -> List[str]:
        ...
        if self.average_relevance < 3.0:
            improvement_areas.append("Query relevance - responses often don't address the user's question")
        
        if self.average_accuracy < 3.0:
            improvement_areas.append("Factual accuracy - responses contain incorrect information")
        
        if self.average_completeness < 3.0:
            improvement_areas.append("Answer completeness - responses are missing key information")
        ...
        if self.average_response_time_ms > 1000:
            improvement_areas.append("Response time - queries are taking too long to process")
        
        return improvement_areas
    
    def compare_with_baseline(self, baseline_metrics: 'EvaluationMetrics') -> Dict[str, Any]:
        """Compare current metrics with a baseline."""
        comparison = {}
        
        # Quality score comparisons
        for metric in ["relevance", "accuracy", "completeness", "helpfulness", "overall"]:
        ...
        if baseline_metrics.average_response_time_ms > 0:
            response_time_change = self.average_response_time_ms - baseline_metrics.average_response_time_ms
            response_time_change_percent = (response_time_change / baseline_metrics.average_response_time_ms) * 100
        ...
        if baseline_metrics.success_rate > 0:
            ...
            success_rate_change_percent = (success_rate_change / baseline_metrics.success_rate) * 100
