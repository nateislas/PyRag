...
"""
    ...
    async def evaluate_batch(
        ...
        """
        self.logger.info(f"Starting batch evaluation of {len(test_results)} test results")
        
        evaluated_results = []
        
        for i, test_result in enumerate(test_results):
            try:
                self.logger.info(f"Evaluating test result {i+1}/{len(test_results)}: {test_result.test_case.id}")
                ...
                self.logger.error(f"Failed to evaluate test result {test_result.test_case.id}: {e}")
                test_result.errors.append(f"Evaluation failed: {e}")
                evaluated_results.append(test_result)
        
        self.logger.info(f"Batch evaluation completed: {len(evaluated_results)} results processed")
