...
from .test_suite import TestResult


class EvaluationReporter:
    """Generate comprehensive evaluation reports."""
    
    def __init__(self, test_suite: TestSuite, metrics: EvaluationMetrics, test_results: List[TestResult]):
    ...
    def generate_markdown_report(self) -> str:
        ...
        report.append(f"**Generated**: {self.report_timestamp.strftime('%Y-%m-%d %H:%M:%S UTC')}")
        ...
        report.append(f"**Evaluation Duration**: {summary['evaluation_info']['duration_seconds']:.2f} seconds")
        report.append("")
        ...
        report.append("")
        report.append(f"**Average Response Time**: {summary['performance']['average_response_time_ms']:.2f} ms")
        if summary['performance']['total_token_usage']:
            report.append(f"**Total Token Usage**: {summary['performance']['total_token_usage']:,}")
            report.append(f"**Average Tokens per Query**: {summary['performance']['average_tokens_per_query']:.1f}")
        report.append("")
        ...
        report.append(f"**Successful Tests**: {summary['test_execution']['successful_tests']}")
        ...
        if summary['category_breakdown']:
            ...
            report.append("")
            report.append("| Category | Count | Avg Relevance | Avg Accuracy | Avg Completeness | Avg Helpfulness | Avg Overall |")
            report.append("|----------|-------|---------------|--------------|------------------|-----------------|-------------|")
        ...
        if summary['difficulty_breakdown']:
            ...
            report.append("")
            report.append("| Difficulty | Count | Avg Relevance | Avg Accuracy | Avg Completeness | Avg Helpfulness | Avg Overall |")
            report.append("|------------|-------|---------------|--------------|------------------|-----------------|-------------|")
        ...
        for category, results in categories.items():
            ...
            report.append("| Test ID | Query | Difficulty | Overall | Response Time | Status |")
            report.append("|---------|-------|------------|---------|---------------|--------|")
            
            for result in results:
                query_preview = result.test_case.query[:40] + "..." if len(result.test_case.query) > 40 else result.test_case.query
                ...
                report.append(f"| {result.test_case.id} | {query_preview} | {difficulty} | {overall} | {response_time} | {status} |")
        ...
        # Footer
        report.append("---")
        report.append(f"*Report generated by PyRAG Evaluation System on {self.report_timestamp.strftime('%Y-%m-%d %H:%M:%S UTC')}*")
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if summary['quality_scores']['relevance'] < 3.0:
            recommendations.append("Improve query understanding and retrieval relevance")
        ...
        if summary['quality_scores']['completeness'] < 3.0:
            recommendations.append("Expand retrieval to include more comprehensive information")
        ...
        if summary['performance']['average_response_time_ms'] > 1000:
            recommendations.append("Optimize search algorithms and reduce response time")
        ...
        for category, scores in summary['category_breakdown'].items():
            if scores['average_overall'] < 3.0:
                recommendations.append(f"Focus on improving {category.replace('_', ' ')} performance")
        
        # Difficulty-based recommendations
        for difficulty, scores in summary['difficulty_breakdown'].items():
            if scores['average_overall'] < 3.0:
                recommendations.append(f"Enhance handling of {difficulty} complexity queries")
...
<body>
    ...
    def print_summary(self) -> None:
        ...
        print(f"⏱️  Response Time: {summary['performance']['average_response_time_ms']:.0f}ms")
