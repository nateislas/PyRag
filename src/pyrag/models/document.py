...
class DocumentChunk(Base):
    ...
    library_version_id: Mapped[int] = mapped_column(ForeignKey("library_versions.id"), nullable=False)
    
    # Content information
    content: Mapped[str] = mapped_column(Text, nullable=False)
    content_type: Mapped[str] = mapped_column(String(50), nullable=False)  # signature, description, example, overview
    
    # Hierarchical organization
    hierarchy_path: Mapped[str] = mapped_column(String(500), nullable=False)  # e.g., "requests.auth.HTTPBasicAuth.__call__"
    hierarchy_level: Mapped[int] = mapped_column(default=0, nullable=False)  # 0=library, 1=module, 2=class, 3=method
    ...
    # Vector storage
    embedding_id: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)  # ID in vector store
    embedding_model: Mapped[Optional[str]] = mapped_column(String(100), nullable=True)
    
    # Additional metadata as JSON
    additional_metadata: Mapped[Optional[dict]] = mapped_column(JSON, nullable=True)  # Parameters, deprecation info, etc.
    
    # Relationships
    library_version: Mapped["LibraryVersion"] = relationship("LibraryVersion", back_populates="chunks")
