...
class ComplianceStatus(Base):
    ...
    # License information
    license_approved: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    license_review_date: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True)
    license_reviewer: Mapped[Optional[str]] = mapped_column(String(100), nullable=True)
    
    # Maintainer communication
    maintainer_contacted: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    maintainer_contact_date: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True)
    maintainer_response: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    
    # Opt-out status
    opt_out_status: Mapped[str] = mapped_column(String(50), default="none", nullable=False)  # none, requested, granted, denied
    ...
    # Relationships
    library: Mapped["Library"] = relationship("Library", back_populates="compliance_status")
...
class UpdateLog(Base):
    ...
    library_id: Mapped[Optional[int]] = mapped_column(ForeignKey("libraries.id"), nullable=True)
    
    # Update information
    update_type: Mapped[str] = mapped_column(String(50), nullable=False)  # initial, version_update, content_update, compliance_check
    status: Mapped[str] = mapped_column(String(50), nullable=False)  # pending, in_progress, completed, failed
    ...
    # Relationships
    library_version: Mapped[Optional["LibraryVersion"]] = relationship("LibraryVersion", back_populates="update_logs")
    
    def __repr__(self) -> str:
        """String representation of the update log."""
        library_name = self.library_version.library.name if self.library_version else "unknown"
