...
async def search_documentation(
    request: SearchRequest, pyrag: PyRAG = Depends(get_pyrag)
) -> SearchResponse:
...
async def get_api_reference(
    request: APIReferenceRequest, pyrag: PyRAG = Depends(get_pyrag)
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"API reference lookup failed: {str(e)}"
        )


@router.get(
    "/api-reference/{library}/{api_path:path}", response_model=APIReferenceResponse
)
async def get_api_reference_get(
    ...
    try:
        ...
        result = await pyrag.get_api_reference(
            library=library, api_path=api_path, include_examples=include_examples
        )
