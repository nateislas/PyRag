...
async def create_library(
    request: LibraryCreateRequest, pyrag: PyRAG = Depends(get_pyrag)
) -> LibraryResponse:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Library creation failed: {str(e)}"
        )


@router.get("/", response_model=List[LibraryResponse])
async def list_libraries(pyrag: PyRAG = Depends(get_pyrag)) -> List[LibraryResponse]:
    """List all libraries."""
    try:
...
async def get_library(
    library_name: str, pyrag: PyRAG = Depends(get_pyrag)
) -> LibraryResponse:
    """Get library details."""
    try:
        ...
        if not status:
            raise HTTPException(
                status_code=404, detail=f"Library not found: {library_name}"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Library retrieval failed: {str(e)}"
        )


@router.post("/{library_name}/documents", response_model=DocumentAddResponse)
async def add_documents(
    library_name: str, request: DocumentAddRequest, pyrag: PyRAG = Depends(get_pyrag)
) -> DocumentAddResponse:
...
async def get_library_status(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Library status retrieval failed: {str(e)}"
        )
