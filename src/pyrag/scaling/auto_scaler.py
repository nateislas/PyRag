...
class AutoScaler:
    ...
    def _make_scaling_decision(self, metrics: Dict[str, Any]) -> ScalingDecision:
        ...
        scale_up_needed = (
            cpu_utilization > self.policy.scale_up_threshold
            or memory_utilization > self.policy.scale_up_threshold
            or response_time > self.policy.target_response_time
            or error_rate > 0.05
            or queue_length > 10  # >5% error rate  # Queue building up
        )
        ...
        if scale_up_needed and self.current_instances < self.policy.max_instances:
            ...
            reason = self._build_scale_up_reason(
                metrics,
                cpu_utilization,
                memory_utilization,
                response_time,
        ...
        scale_down_needed = (
            cpu_utilization < self.policy.scale_down_threshold
            and memory_utilization < self.policy.scale_down_threshold
            and response_time < self.policy.target_response_time * 0.5
            and error_rate < 0.01  # Well below target
            and queue_length < 2  # Low error rate  # No queue buildup
        )
        ...
        if scale_down_needed and self.current_instances > self.policy.min_instances:
            ...
            reason = self._build_scale_down_reason(
                metrics,
                cpu_utilization,
                memory_utilization,
                response_time,
    ...
    def _build_scale_up_reason(
        ...
        if cpu > self.policy.scale_up_threshold:
            reasons.append(
                f"CPU utilization {cpu:.1f}% > {self.policy.scale_up_threshold}%"
            )

        if memory > self.policy.scale_up_threshold:
            reasons.append(
                f"Memory utilization {memory:.1f}% > {self.policy.scale_up_threshold}%"
            )

        if response_time > self.policy.target_response_time:
            reasons.append(
                f"Response time {response_time:.1f}ms > {self.policy.target_response_time}ms"
            )
    ...
    def _build_scale_down_reason(
        ...
        if cpu < self.policy.scale_down_threshold:
            reasons.append(
                f"CPU utilization {cpu:.1f}% < {self.policy.scale_down_threshold}%"
            )

        if memory < self.policy.scale_down_threshold:
            reasons.append(
                f"Memory utilization {memory:.1f}% < {self.policy.scale_down_threshold}%"
            )
    ...
    async def _execute_scale_up(self, decision: ScalingDecision):
        ...
        logger.info(
            f"Scaling up: {decision.current_instances} -> {decision.target_instances} instances"
        )
    ...
    async def _execute_scale_down(self, decision: ScalingDecision):
        ...
        logger.info(
            f"Scaling down: {decision.current_instances} -> {decision.target_instances} instances"
        )
    ...
    async def manual_scale(self, target_instances: int):
        ...
        if (
            target_instances < self.policy.min_instances
            or target_instances > self.policy.max_instances
        ):
            raise ValueError(
                f"Target instances {target_instances} outside policy range [{self.policy.min_instances}, {self.policy.max_instances}]"
            )
