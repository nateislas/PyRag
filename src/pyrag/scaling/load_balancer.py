...
class LoadBalancer:
    ...
    def __init__(
        ...
        for backend_config in backends:
            ...
            self.backends[backend_id] = BackendInfo(
                ...
                error_rate=0.0,
                load_factor=1.0,
                max_concurrent_queries=backend_config.get(
    ...
    def _select_backend(
        self, available_backends: List[BackendInfo], query_info: QueryInfo
    ) -> Optional[str]:
        """
        ...
        for backend in available_backends:
            ...
            load_weight = max(
                0.1, 1.0 - (backend.current_queries / backend.max_concurrent_queries)
            )
    ...
    async def execute_query(self, query_info: QueryInfo, query_func: callable) -> Any:
        """
        ...
        for attempt in range(self.max_retries):
            ...
            try:
                ...
                self.backends[backend_id].error_rate = max(
                    0.0, self.backends[backend_id].error_rate - 0.01
                )
                ...
                backend.error_rate = backend.total_errors / max(
                    1, backend.total_queries
                )
                ...
                if backend_id in self.backends:
                    self.backends[backend_id].current_queries = max(
                        0, self.backends[backend_id].current_queries - 1
                    )
