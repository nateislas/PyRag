...
class HealthChecker:
    ...
    async def _update_service_metrics(self, service_id: str, result: HealthCheckResult):
        ...
        if response_times:
            ...
            metrics.response_time_p95 = sorted_times[p95_index] if p95_index < len(sorted_times) else sorted_times[-1]
            metrics.response_time_p99 = sorted_times[p99_index] if p99_index < len(sorted_times) else sorted_times[-1]
        ...
        if metrics.status == HealthStatus.UNHEALTHY and metrics.consecutive_failures == self.max_consecutive_failures:
            await self._trigger_alert(service_id, "Service marked as unhealthy", metrics)
    
    async def _trigger_alert(self, service_id: str, message: str, metrics: HealthMetrics):
