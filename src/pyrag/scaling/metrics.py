...
class MetricsCollector:
    ...
    def __init__(
        ...
        # Metrics storage
        self.performance_history: deque = deque(maxlen=10000)  # ~2.8 hours at 1s intervals
    ...
    def end_query_tracking(
        ...
        total_queries = self.query_counters["successful"] + self.query_counters["failed"]
        if total_queries > 0:
            self.query_counters["avg_duration"] = (
                (self.query_counters["avg_duration"] * (total_queries - 1) + query_metrics.duration) / total_queries
            )
    ...
    def export_metrics(self, format: str = "json") -> str:
        ...
        if format.lower() == "json":
            ...
            lines = ["timestamp,cpu_utilization,memory_utilization,response_time_avg,throughput_qps,error_rate"]
            
            for metric in list(self.performance_history)[-100:]:  # Last 100 metrics
                lines.append(f"{metric.timestamp},{metric.cpu_utilization},{metric.memory_utilization},{metric.response_time_avg},{metric.throughput_qps},{metric.error_rate}")
...
class PerformanceMonitor:
    ...
    async def check_alerts(self):
        ...
        if current_metrics["memory_utilization"] > self.thresholds["memory_utilization"]:
            alerts.append({
                "type": "high_memory",
