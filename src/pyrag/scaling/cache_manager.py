...
class CacheManager:
    ...
    def set(
        ...
        while (
            (len(self.cache) >= self.max_entries or 
             self.stats["total_size"] + size_bytes > self.max_size_bytes) and
            len(self.cache) > 0
        ):
            self._evict_entry()
    ...
    def get_stats(self) -> CacheStats:
        ...
        oldest_entry = min(entry.created_at for entry in self.cache.values()) if self.cache else 0.0
        newest_entry = max(entry.created_at for entry in self.cache.values()) if self.cache else 0.0
